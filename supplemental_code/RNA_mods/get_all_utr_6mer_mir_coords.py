...
# this script pulls all 3'UTR sequences from a GTF file and searches them for miRNA 6mer seeds from TargetScan 
...
with open(sys.argv[3]) as mirs:
	for line in mirs:
		line=line.strip().split()
		mir,rev_string=line[0],rev_complement_6mer(line[1])
...
for utr in utr_seqs_minus:
	...
	for seed in seed_list:
		res = [i for i in range(len(seq)) if seq.startswith(seed, i)]
		if res:
			res_minus=[len(seq)-i-6 for i in res] 
			if seed not in found_dict:
...
# convert locations of 6mers relative to 3'UTR starts to genomic coordinates, accounting for the fact that some seeds may span exon-exon junctions
				...
				for idx,size in enumerate(sizes):
					...
					if site>size:
						...
						if end_site <= size: # if seed site ends in the same exon as it started
							...
							remaining=6-(seed_end-seed_start)
...
# write out the genomic coordinates of 6mer seeds to a bed6 file
