...
# create a dictionary of 3'UTR sequences based separated by strand
		...
		for site in seed_coords:
			...
			if utr_coords[name][-1]=='0,':
				...
				if utr_coords[name][0]=='MtDNA':
					chrom,strand='chrM',utr_coords[name][5]
				else:
					chrom,strand='chr'+utr_coords[name][0],utr_coords[name][5]
				...
				for idx,size in enumerate(sizes):
					...
					if site>size:
						...
						if end_site <= size: # if seed site ends in the same exon as it started
							...
							add=[chrom,str(seed_start),str(seed_end),seed,str(remaining),strand]
							add2=[chrom,str(seed_start_2),str(seed_end_2),seed,'60',strand]
